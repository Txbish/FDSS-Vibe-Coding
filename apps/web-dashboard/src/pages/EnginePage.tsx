...
import { runSimulation, runBranch } from '../api';
...
export function EnginePage() {
  ...
  const handleBranch = async (
    ...
    setError(null);
    try {
      const branchOut = await runBranch(input, branchDay, modified);
      setBranchResult(branchOut);
      // Also set the baseline result so metrics stay visible
      setResult(branchOut.baseline);
    } catch (err) {
            ...
            {result && !loading && (
              <>
                <div className="card-grid">
                  ...
                  />
                  {result.totalTaxPaid !== undefined && result.totalTaxPaid > 0 && (
                    <MetricCard
                      label="Total Tax Paid"
                      value={`$${result.totalTaxPaid.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`}
                    />
                  )}
                  {result.monteCarloRuns !== undefined && result.monteCarloRuns > 1 && (
                    <MetricCard label="Monte Carlo Runs" value={String(result.monteCarloRuns)} />
                  )}
                </div>
                ...
                {result.snapshots.length > 0 &&
                  (() => {
                    ...
                    const hasExtras =
                      lastSnap.shockClusteringDensity !== undefined ||
                      lastSnap.recoverySlope !== undefined ||
                      lastSnap.taxPaid !== undefined;
                    if (!hasExtras) return null;
                    return (
                      <div className="status-grid" style={{ marginTop: '1.5rem' }}>
                        {lastSnap.shockClusteringDensity !== undefined && (
                          <article className="status-card">
                            <div className="card-label">Shock Clustering Density</div>
                            <div className="card-value">
                              {lastSnap.shockClusteringDensity.toFixed(3)}
                            </div>
                          </article>
                        )}
                        {lastSnap.recoverySlope !== undefined && (
                          <article className="status-card">
                            <div className="card-label">Recovery Slope</div>
                            <div className="card-value">{lastSnap.recoverySlope.toFixed(2)}</div>
                          </article>
                        )}
                        {lastSnap.taxPaid !== undefined && lastSnap.taxPaid > 0 && (
                          <article className="status-card">
                ...
                {/* Branch Comparison */}
                {branchResult && branchResult.comparison && (
                  <motion.div
                  ...
                  >
                    ...
                    <p className="branch-summary">{branchResult.comparison.summary}</p>
                    <div className="card-grid">
                      <MetricCard
                        label="Balance Delta"
                        value={`${branchResult.comparison.balanceDelta >= 0 ? '+' : ''}$${branchResult.comparison.balanceDelta.toLocaleString(undefined, { maximumFractionDigits: 2 })}`}
                        variant={branchResult.comparison.balanceDelta >= 0 ? 'success' : 'danger'}
                      />
                      <MetricCard
                        label="Collapse Prob. Delta"
                        value={`${branchResult.comparison.collapseProbDelta >= 0 ? '+' : ''}${(branchResult.comparison.collapseProbDelta * 100).toFixed(1)}%`}
                        variant={
                          branchResult.comparison.collapseProbDelta <= 0 ? 'success' : 'danger'
                        }
                      />
                      <MetricCard
                        label="Credit Score Delta"
                        value={`${branchResult.comparison.creditScoreDelta >= 0 ? '+' : ''}${branchResult.comparison.creditScoreDelta.toFixed(0)}`}
                        variant={
                          branchResult.comparison.creditScoreDelta >= 0 ? 'success' : 'danger'
                        }
                      />
                      <MetricCard
                        label="NAV Delta"
                        value={`${branchResult.comparison.navDelta >= 0 ? '+' : ''}$${branchResult.comparison.navDelta.toLocaleString(undefined, { maximumFractionDigits: 2 })}`}
                        variant={branchResult.comparison.navDelta >= 0 ? 'success' : 'danger'}
                      />
                      <MetricCard
                        label="Vibe Improved?"
                        value={branchResult.comparison.vibeImproved ? 'Yes ✓' : 'No ✗'}
                        variant={branchResult.comparison.vibeImproved ? 'success' : 'warning'}
                      />
                    </div>
                  </motion.div>
                )}
